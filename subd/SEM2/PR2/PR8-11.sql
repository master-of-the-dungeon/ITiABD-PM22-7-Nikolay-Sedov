CREATE OR REPLACE FUNCTION get_formatted_passenger_info(ticket_number varchar)
RETURNS varchar AS $$
DECLARE
    passenger_info record;
    formatted_name text;
    formatted_contact_data text;
BEGIN
    -- Получаем данные пассажира по номеру билета
    SELECT passenger_name, contact_data INTO passenger_info
    FROM passengers
    WHERE ticket_no = ticket_number;

    -- Преобразовываем имя пассажира в нужный формат
    -- Здесь предполагается, что имя и фамилия отделены пробелом
    SELECT initcap(SPLIT_PART(passenger_info.passenger_name, ' ', 1)) || ' ' ||
           initcap(SPLIT_PART(passenger_info.passenger_name, ' ', 2))
    INTO formatted_name;

    -- Форматируем контактные данные
    -- Предполагаем, что контактные данные уже в нужном формате и их просто нужно добавить к строке
    formatted_contact_data := ', contact_data: ' || passenger_info.contact_data;

    -- Конкатенируем полученные строки
    RETURN formatted_name || formatted_contact_data;
END;
$$ LANGUAGE plpgsql;

-- Вызов функции с конкретным номером билета, например '123456'
SELECT get_formatted_passenger_info('123456');
