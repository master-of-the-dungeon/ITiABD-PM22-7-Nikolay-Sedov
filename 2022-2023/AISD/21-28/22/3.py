...
def method_of_midpoint(f, a, b, tol=1e-6):
    c = (a + b) / 2
    while True:
        c = (a * f(b) - b * f(a)) / (f(b) - f(a))
        if abs(f(c)) < tol:
        ...
        if f(a) * f(c) < 0:
            ...
            a = c
def method_of_chords(f, a, b, tol=1e-6):
    c = (a*f(b) - b*f(a)) / (f(b) - f(a))
    while abs(f(c)) > tol:
        if f(c) * f(a) < 0:
            b = c
...
def method_of_newton(f, x0, tol=1e-6):
    x = x0
    while abs(f(x)) > tol:
...
x_mp = method_of_midpoint(f, a, b)
x_ch = method_of_chords(f, a, b)
x_nt = method_of_newton(f, a)

print("Минимум методом средней точки:", x_mp)
print("Минимум методом хорд:", x_ch)
print("Минимум методом Ньютона:", x_nt)
x = np.linspace(a, b, 1000)
y = f(x)
